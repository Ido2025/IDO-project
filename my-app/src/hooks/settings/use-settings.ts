    ...
    const onChangePassword = handleSubmit(async (Values) => {
        try {
            ...
            if (updated) {
                reset()
                setLoading(false)
                toast({ title: 'Success', description: updated.massage })
            }
        } catch (error) {
            console.log(error)
        }
...
export const useHelpDesk = (id: string) => {
  ...
  export const useFilterQuestions = (id: string) => {
    ...
    // Hook to fetch questions for the current domain and set the component state
const onGetQuestions = async () => {
  setLoading(true) // Start loading state while the questions are being fetched

  // Call the API to get the questions for the domain
  const questions = await onGetAllFilterQuestions(id)

  // If questions are successfully fetched, update the state
  if (questions) {
    setIsQuestions(questions.questions) // Set the questions into the component's state
    setLoading(false) // Stop loading state once questions are loaded
  }
}
